...
model_type = cell_type + "testfordbuse"
...
keep_prob = 0.8     # if no dropout wanted: 1
keep_prob_test = 1.0

n_train = 500
n_test = 16 #1000 but changed for testing

print("Number of layers: {}\nSize of layers: {}".format(n_layers, layer_sizes))
...
def dropout(cell):
    dropout_cell = tf.contrib.rnn.DropoutWrapper(cell, output_keep_prob=p_dropout)
    return dropout_cell
...
p_dropout = tf.placeholder(dtype=tf.float32, name="keep_prob")

with tf.name_scope("data"):
    x = tf.placeholder(tf.float32, shape=[None, window, n_inputs], name="x")
    y = tf.placeholder(tf.float32, shape=[None, window, n_outputs], name="y")
...
with tf.name_scope("recurrent_layer"):
    ...
    # if bidirectional:             # is standard
    stacked_cells_fw = [dropout(set_cell(cell_type, size)) for size in layer_sizes]
    stacked_cells_bw = [dropout(set_cell(cell_type, size)) for size in layer_sizes]
        
    rnn_output, states_fw, states_bw = tf.contrib.rnn.stack_bidirectional_dynamic_rnn(
                                            stacked_cells_fw, stacked_cells_bw, x, dtype=tf.float32)
    
    #~ stacked_cells_fw = tf.contrib.rnn.MultiRNNCell(stacked_cells_fw)
    #~ stacked_cells_bw = tf.contrib.rnn.MultiRNNCell(stacked_cells_bw)                    
    #~ outputs, states = tf.nn.bidirectional_dynamic_rnn(stacked_cells_fw, stacked_cells_bw, x, dtype=tf.float32)
...
stacked_rnn_output = tf.reshape(rnn_output, [-1, layer_size * 2])           # 2 cause bidirectional
...
#~ with tf.name_scope("final_fully_connected"):
    ...
    #~ stacked_outputs = tf.add(tf.multiply(tf.reshape(rnn_output, [-1, n_outputs]), w), b)    
...
with tf.name_scope("TensorBoard"):
    ...
    #~ loss_ep_summ = tf.summary.scalar("loss_ep", loss)
    #~ acc_ep_summ = tf.summary.scalar("accuracy_ep", accuracy)
...
with tf.Session() as sess:
    ...
    z = tf.shape(x)
    print("Shape data batch: ", sess.run(z, feed_dict={x: train_x[: batch_size]}))
    print("Shape test data batch: ", sess.run(z, feed_dict={x: test_x}))
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for batch in range(n_batches):
            step += 1
            feed_dict = {x: train_x[batch * batch_size : (batch + 1) * batch_size], y: train_y[batch * batch_size: (batch + 1) * batch_size], p_dropout: keep_prob}
    ...
    feed_dict_test = {x: test_x, y: test_y, p_dropout: keep_prob_test}
...
## Helper functions
# for classes:
# make reshape function
# make accuracy function    - calc before and after

def show_loss_plot(loss_history, n_epochs):
...
def _create_summaries():    # has self
    with tf.name_scope("summaries"):
        tf.summary.scalar("loss", self.loss)
        tf.summary.scalar("accuracy", self.accuracy)
        tf.summary.histogram("loss histogram", self.loss_history)
        self.summary_op = tf.summary.merge_all()

#TODO: 
# implement in session as
