...
def metrics(true_labels, predicted_labels):
    ...
    """
    # check if labels can be compared
    ...
    # calculate precision and recall if not zero
    try:
...
def weighted_f1(precision, recall, n, N):
    ...
    try:
        ...
        print("Precision, recall or both could not be calculated. Unable of calculating weighted F1.")
        wF1 = 0
    return wF1
...
def set_nbatches(category):
    if category == "squiggles":
        n_batches = max_seq_length // batch_size // window      
    if category == "trainingreads":
...
def build_rnn(extra=True):   
    ...
    if extra:
        print("\n", "RNN output get shape: ", rnn_output.get_shape())
        print("Stacked RNN get shape: ", stacked_rnn_output.get_shape())   
